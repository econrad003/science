"""
tests.gaussian - Gaussian (root -1) rational numbers tests
Eric Conrad
Copyright ©2026 by Eric Conrad.  Licensed under GPL.v3.

DESCRIPTION

    This module tests rational and integer division in ℚ(i).
...
from utilities.gauss_frac import GaussianFrac as GFrac

print("TESTING GAUSSIAN RATIONALS ℚ(i) MODULE")
print()
ONE = GFrac(1, 0, 1)
print(f"{ONE} = 1")
assert ONE == 1
...
I = GFrac(0, 1, 1)
print(f"{I} = i")
assert I.sqnorm == 1, f"{I.sqnorm} == 1"
assert abs(I) == 1
...
print()
print("Here we verify that all points are within 1 of Gaussian integers...")
...
print()
print("Notes:")
print("    (1) Rounding is safe.  The maximum distance to the nearest neighbor")
print(f"        is 1/√2 (or {sqrt(2)/2}) which is less than 1.")
print("    (2) Directed rounding is unsafe.  The distance to an opposite")
print(f"        corner is √2 (or {sqrt(2)}) which is larger than 1.")

print()
print("SECTION 2. RATIONAL DIVISION")
print()

def randgauss(n:int, d:int=1, no_zeros:bool=False) -> GFrac:
    """generate a random Gaussian fraction
...
n = 0
while n < 100:
    a = randgauss(100, d=50)
    b = randgauss(100, d=50, no_zeros=True)
    q = a / b
...
common = I*7 + 25
a = dividend = randgauss(100000, d=1, no_zeros=True) * common
b = divisor = randgauss(10000, d=1, no_zeros=True) * common
