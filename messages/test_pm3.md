...
```
    ...
    In the text box that appears, enter "logic.tests.pm3" (without quotes).
```
...
7. Notes on implication and negation in the divisor logics
8. ℒ(4) (the lattice of divisors of 4, i.e. {1, 2, 4})
9. ℒ(6) (the lattice of divisors of 6, i.e. {1, 2, 3, 6})
10. ℒ(30) (the lattice of divisors of 30)
11. ℒ(60) (the lattice of divisors of 60)
12. ℒ(8) (the lattice of divisors of 8)
13. ℒ(16) (the lattice of divisors of 16)
14. ℒ(36) (the lattice of divisors of 36)
15. isomorphisms
...
```
   ...
   T16.3: (p⊃~q)⊃(q⊃~p)                        contraposition
   T16.4: (~p⊃q)⊃(~q⊃p)                        contraposition
...
Finally we state seven derived Rules:
```
   DR1: If ⊢p and ⊢q, then ⊢(p∧q).           adjunction
   DR2: If ⊢(p⊃q) and ⊢~q, then ⊢~p.         modus tollens 1
   DR3: If ⊢(~p⊃~q) and ⊢q, then ⊢p.         modus tollens 2
   DR4: If ⊢(p⊃~q) and ⊢q, then ⊢~p.         modus tollens 3
   DR5: If ⊢(~p⊃q) and ⊢~q, then ⊢p.         modus tollens 4
   DR6: If ⊢(~p⊃p), then ⊢p.                 contradiction 1
   DR7: If ⊢(p⊃~p), then ⊢~p.                contradiction 2
                     contradiction
	--> All the transformations are valid.
```
The *modus tollens* rules can be deduced from the contrapositive theorems T16.1 through T16.4.  In a system where natural deduction works (as it does in PM), rules DR2 through DR5 and and the contrapositive theorems T16.1 through T16.4 are equivalent.

The contradiction rules DR6 and DR7 are consequences of T15.1 and T15.2 (*reductio ad absurdum*).
...
The last two failures are in the contradiction rules:
```
   DR6: If ⊢(~p⊃p), then ⊢p.
...
```
    ...
    Derived Rules:
        ...
        DR7: If ⊢(p⊃~p), then ⊢~p.                contradiction 2
	       Conclusion ~p = J when |p|=I and premises hold.
```
...
* the discrepances differ from those in Ł₃.

We detected several additional discrepancies:
```
    Total number of invalid items: 13
```
...
Theorem Schemas:
   ...
   T16.3: (p⊃~q)⊃(q⊃~p)                        contraposition
   T16.4: (~p⊃q)⊃(~q⊃p)                        contraposition
	not valid: |q|=F, |p|=I, result=I              (#7)
   T17.1: p⊃~~p                                double negation
   T17.2: ~~p⊃p                                double negation
	   not valid: |p|=I, result=I                  (#8)
   T17.3: ~~~p⊃~p                              double negation
   T17.4: p≡~~p                                double negation
	   not valid: |p|=I, result=I                  (#9)
   T26: (p∨q)≡((p⊃q)⊃q)                      alternate def: Apq
	   not valid: |q|=F, |p|=I, result=I           (#10)
```
...
Contrapositives are valid in some cases (T16.1 and T16.3) but fail in some others  (T16.2 and T16.4).  Negation in G₃ once again breaks symmetry.  All four forms are valid in Ł₃.

Double negation is valid when proving a negative (T17.1 and T17.3) but not when proving a positive (T17.2) or when one side of an equivalence is positive (T17.4).
...
Our database fails in three more spots:
```
Derived Rules:
   DR1: If ⊢p and ⊢q, then ⊢(p∧q).                 adjunction
   DR2: If ⊢(p⊃q) and ⊢~q, then ⊢~p.               modus tollens 1
   DR3: If ⊢(~p⊃~q) and ⊢q, then ⊢p.               modus tollens 2
	 Conclusion p = I when |p|=I, |q|=T and premises hold.     (#11)
   DR4: If ⊢(p⊃~q) and ⊢q, then ⊢~p.               modus tollens 3
   DR5: If ⊢(~p⊃q) and ⊢~q, then ⊢p.               modus tollens 4
	 Conclusion p = I when |p|=I, |q|=F and premises hold.     (#12)
   DR6: If ⊢(~p⊃p), then ⊢p.                       contradiction 1
	 Conclusion p = I when |p|=I and premises hold.            (#13)
   DR7: If ⊢(p⊃~p), then ⊢~p.                      contradiction 2
```

Our result for DR6 agrees with T15.2.  But note that rule DR7 is *valid*

Note also two of the forms of *modus tollens*, namely DR3 and DR5, are not valid; however DR2 and DR4 are valid.  DR2 and DR4 yield negative results, but the results of failed rules DR1 and DR3 are positive.  In the Gödel-Dummett algebras, and also in the intuitionistic propositional calculus, ~p should be read as "*it is impossible* that p", and not merely as "*it is false* that p".
```
...
## Notes on implication and negation in divisor logics
...
In our first attempt we tried this completion:
```
...
Below we focus on the results of our attempts with implication defined using points (1), (2) and (3.3).

## 8. ℒ(4), the lattice of divisors of 4
...
There are ten differences from the definitions, axioms, theorems, and rules that are recorded in the current database:
```
Total number of invalid items: 10
```
...
The axioms in the *Principia Mathematica* and *modus ponens* are all valid in ℒ(4).  Lattice properties all hold, and material implication is defined as in T24.1 through T24.4.

Material equivalence is an equivalence relation, and negation, disjunction, conjunction, material implication, and material equivalence are all monotone with respect to material equivalence.

(In earlier tests, when we used the GCD to handle failures in implication, negation was not monotone.  The definition here appears to be an improvement.)

Here are the remaining discrepancies:
```
Theorem Schemas:
    T5: (p⊃(q⊃r))⊃((p⊃q)⊃(p⊃r))              importation
       not valid: |p|=2, |q|=2, |r|=1, result=2    (#2)
    T9: ((p⊃q)⊃p)⊃p                          Pierce
       not valid: |p|=2, |q|=1, result=2           (#3)
   T11: (p⊃(q⊃r))⊃((p∧q)⊃r)                  importation
	   not valid: |q|=2, |p|=2, |r|=1, result=2    (#4)
   ...
   T15.2: (~p⊃p)⊃p                             reductio ad absurdum
	   not valid: |p|=2, result=2                  (#8)
```
These seven all fail in Ł₃.  Since T5 fails, the system does not support natural deduction.

(Note that T5 (used to prove natural deduction) does succeed when the GCD is used to fill in failures for material implication.  However, contrapositives failed in that algebra.)

T11 (importing conjunction) also fails in the Lewis algebra S2.

The contradiction rules, derived from RAA (*i.e.*, *reductio ad absurdum*) using modus ponens, both fail, as expected.
```
Derived Rules:
   DR6: If ⊢(~p⊃p), then ⊢p.                  contradiction 1
	 Conclusion p = 2 when |p|=2 and premises hold.    (#9)
   DR7: If ⊢(p⊃~p), then ⊢~p.                 contradiction 2
	 Conclusion ~p = 2 when |p|=2 and premises hold.   (#10)
```

If we look at the square of any prime number p, we get an isomorphic structure.  This particular lattice of divisors is a 3-valued propositional caclulus which is not isomorphic to G₃ but might be isomorphic to Ł₃. The relationship with Ł₃ needs to be investigated more closely.

# 9. ℒ(6), lattice of divisors of 6
...
```
                     ...
                     2=TF   3=FT            2=I
                       \    /                |
                        ...
                        ℒ(6)                ℒ(4)
```
The negation operator was defined as `|~p|=4/|p|` in ℒ(4), and as `|~p|=6/|p|` in ℒ(6).  This generalizes negation in the Łukasiewicz algebras to multiple worlds.  For ℒ(n), the rule is `|~p|=n/|p|`.  The real issue is how to *best* define implication when it fails, *i.e.* in the lower diagonal.  After some experimentation with oher candidates, we have elected to use the following:
```
                    ┏
                    ┃  n, if |p| ≤ |q|, and
            |p⊃q| = ┫
...
The number of failures detected in our database was 0.  So the theorems for this lattice are precisely those of the classical propositional calculus.  But the lattice is not isomorphic to the two-values {F, T} lattice.  (We would need additional connectives to make the distinction.)

## 10. ℒ(30), lattice of divisors of 30
...
There we no failures.  As with ℒ(6), the theorems are precisely the theorems of the classical propositional calculus.  We would need additional connectives to distinguish the lattices ℒ(2) (classical propositional calculus), ℒ(6) (two independent worlds), and ℒ(30) (three independent worlds).


## 11. ℒ(60), lattice of divisors of 60

Since 60=2²⋅3⋅5, we have a three world scenario where world 2 is ternary (T,I,F) and worlds 3 and 5 are binary (T,F).  The 10 failures seem to be the same as what goes wrong in ℒ(4) (representing a single ternary world).

We save the problems here:
```
Definitions:
    D2: (p⊃q) := (~p∨q)                      implication
	   not valid: |q|=2, |p|=2, result=30
	--> One definition is not valid.
Axiom Schemas:
	--> All the axiom schemas are valid.
Rules of Transformation:
	--> All the transformations are valid.
Theorem Schemas:
    T5: (p⊃(q⊃r))⊃((p⊃q)⊃(p⊃r))              importation
	   not valid: |p|=2, |r|=1, |q|=2, result=30
    T9: ((p⊃q)⊃p)⊃p                          Pierce
	   not valid: |p|=2, |q|=1, result=30
   T11: (p⊃(q⊃r))⊃((p∧q)⊃r)                  importation
	   not valid: |q|=2, |r|=1, |p|=2, result=30
   T13.1: p∨~p                                 tertium non datur
   ...
   T15.1: (~p⊃p)⊃p                             reductio ad absurdum
	   not valid: |p|=2, result=30
   T16: (p⊃q)⊃(~q⊃~p)                        CCpqCNqNp           contraposition
   T16: (~p⊃~q)⊃(q⊃p)                        CCNpNqCqp           contraposition
   T16: (p⊃~q)⊃(q⊃~p)                        CCpNqCqNp           contraposition
   T16: (~p⊃q)⊃(~q⊃p)                        CCNpqCNqp           contraposition
   T17: p⊃~~p                                CpNNp              double negation
   T17: ~~p⊃p                                CNNpp              double negation
   T17: p≡~~p                                EpNNp              double negation
   T17: ~~~p⊃~p                              CNNNpNp            double negation
   T18: p≡(p∨p)                              EpApp                    tautology
   T18: p≡(p∧p)                              EpKpp                    tautology
   T19: p⊃(p∨q)                              CpApq               simplification
   T19: (p∧q)⊃p                              CKpqp               simplification
   T20: (p∨q)≡(q∨p)                          EApqAqp                commutation
   T20: (p∧q)≡(q∧p)                          EKpqKqp                commutation
   T21: (p∨(q∨r))≡((p∨q)∨r)                  EApAqrAApqr            association
   T21: (p∧(q∧r))≡((p∧q)∧r)                  EKpKqrKKpqr            association
   T22: (p∧(q∨r))≡((p∧q)∨(p∧r))              EKpAqrAKpqKpr         distribution
   T22: (p∨(q∧r))≡((p∨q)∧(p∨r))              EApKqrKApqApr         distribution
   T23: (p∨(p∧q))≡p                          EApKpqp                 absorption
   T23: (p∧(p∨q))≡p                          EKpApqp                 absorption
   T24: (p⊃q)⊃(p≡(p∧q))                      CCpqEpKpq                   subset
   T24: (p≡(p∧q))⊃(p⊃q)                      CEpKpqCpq                   subset
   T24: (p⊃q)⊃(q≡(p∨q))                      CCpqEqApq                   subset
   T24: (q≡(p∨q))⊃(p⊃q)                      CEqApqCpq                   subset
   T25: (p∨F)≡p                              EApFp                      binding
   T25: (p∧T)≡p                              EKpTp                      binding
   T25: (p⊃F)≡~p                             ECpFNp                     binding
   T26: (p∨q)≡((p⊃q)⊃q)                      EApqCCpqq       alternate def: Apq
	--> 7 theorem schemas are not valid.
Derived Rules:
   DR1: If ⊢p and ⊢q, then ⊢(p∧q).                                   adjunction
   DR2: If ⊢(p⊃q) and ⊢~q, then ⊢~p.                            modus tollens 1
   DR3: If ⊢(~p⊃~q) and ⊢q, then ⊢p.                            modus tollens 2
   ...
   DR7: If ⊢(p⊃~p), then ⊢~p.                                   contradiction 2
	 Conclusion ~p = 30 when |p|=2 and premises hold.
	--> 2 transformations are not valid.
Total number of invalid items: 10
```

The failures of D1. T9, T13.1 (the strong form the law of the excluded middle), T14. T15.1, T15.2, DR6 and DR7 are all very desirable features in a system with three or more truth values.

The failure of T5 indicates that material implication, as defined, does not support natural deduction.

The failure to importation meets (T11) and the corresponding success of export them is characteristic of the Łukasiewicz algebras.

**NOTE:**

> I suspect that with the current definition of implication, that ℒ(4) and Ł₃ are isomorphic.  ℒ(4) is isomorphic to a sublattice of ℒ(60), namely the sublattice induced by the set {1, 30, 60}. 

## 12. ℒ(8), lattice of divisors of 8

The discrepancies from our classical database are the same one that occur with the Łukasiewicz algebras, and also with ℒ(4) and ℒ(60). See the discussion under these headers for more information,

The discrepancies:
```
Definitions:
    D2: (p⊃q) := (~p∨q)                      implication
    	   not valid: |p|=2, |q|=2, result=4
	--> One definition is not valid.
Axiom Schemas:
	--> All the axiom schemas are valid.
Rules of Transformation:
...
Theorem Schemas:
    T5: (p⊃(q⊃r))⊃((p⊃q)⊃(p⊃r))              importation
	   not valid: |p|=2, |q|=2, |r|=1, result=4
    T9: ((p⊃q)⊃p)⊃p                          Pierce
	   not valid: |p|=2, |q|=1, result=4
   T11: (p⊃(q⊃r))⊃((p∧q)⊃r)                  importation
	   not valid: |p|=2, |q|=2, |r|=1, result=4
   T13.1: p∨~p                                 tertium non datur
	   not valid: |p|=2, result=4
   T14: ~(p∧~p)                              contradiction
	   not valid: |p|=2, result=4
   T15.1: (p⊃~p)⊃~p                            reductio ad absurdum
	   not valid: |p|=2, result=4
...
Derived Rules:
   DR6: If ⊢(~p⊃p), then ⊢p.                 contradiction 1
	 Conclusion p = 4 when |p|=4 and premises hold.
   DR7: If ⊢(p⊃~p), then ⊢~p.                contradiction 2
	 Conclusion ~p = 4 when |p|=2 and premises hold.
	--> 2 transformations are not valid.
Total number of invalid items: 10
```
...
> I suspect that with the current definition of implication, that ℒ(8) and Ł₄ are isomorphic. 


## 13. ℒ(16), lattice of divisors of 16

The discrepancies from our classical database are the same one that occur with the Łukasiewicz algebras, and also with ℒ(4), ℒ(8) and ℒ(60). See the discussion under these headers for more information,

The discrepancies:
```
Definitions:
    D2: (p⊃q) := (~p∨q)                      implication
	   not valid: |q|=2, |p|=2, result=8
	--> One definition is not valid.
Axiom Schemas:
	--> All the axiom schemas are valid.
Rules of Transformation:
...
Theorem Schemas:
    T5: (p⊃(q⊃r))⊃((p⊃q)⊃(p⊃r))              importation
	   not valid: |p|=2, |q|=2, |r|=1, result=8
    T9: ((p⊃q)⊃p)⊃p                          Pierce
	   not valid: |p|=2, |q|=1, result=8
   T11: (p⊃(q⊃r))⊃((p∧q)⊃r)                  importation
	   not valid: |q|=2, |p|=2, |r|=1, result=8
   T13.1: p∨~p                                 tertium non datur
	   not valid: |p|=2, result=8
   T14: ~(p∧~p)                              contradiction
	   not valid: |p|=2, result=8
   T15.1: (p⊃~p)⊃~p                            reductio ad absurdum
	   not valid: |p|=2, result=8
...
Derived Rules:
   DR6: If ⊢(~p⊃p), then ⊢p.                 contradiction 1
	   Conclusion p = 4 when |p|=4 and premises hold.
   DR7: If ⊢(p⊃~p), then ⊢~p.                contradiction 2
	   Conclusion ~p = 8 when |p|=2 and premises hold.
	--> 2 transformations are not valid.
Total number of invalid items: 10
```
...
> I suspect that with the current definition of implication, that ℒ(16) and Ł₅ are isomorphic. 

## 14. ℒ(36), lattice of divisors of 36

36 = 4 x 9 = 2² x 3².  The lattice is a model of two independent (or so we are led to believe!) ternary worlds.

Assuming that the worlds are indeed independent and assuming that ℒ(4) and Ł₃ are isomorphic (*i.e.* structurally identical), then ℒ(36) should have the same theorems and rules as both ℒ(4) and Ł₃.  Note that ℒ(36) is not structurally the same as ℒ(4), but systems of connectives based on negation and material implication alone are not sufficient to detect the differences.  We can define joins using T26, meets using D1, and material equivalences using D3.  So axiomatizations of ℒ(4) and Ł₃ can start with the following definitions:
```
Reasonable Definitions in ℒ(4) and Ł₃:
   T26: (p∨q) := ((p⊃q)⊃q)                   disjunction
    D1: (p∧q) := ~(~p∨~q)                    conjunction
    D3: (p≡q) := ((p⊃q)∧(q⊃p))               equivalence
```

The discrepancies (disagreements in the database with the classical case):
```
Definitions:
    D2: (p⊃q) := (~p∨q)                      implication
	   not valid: |q|=2, |p|=2, result=18
	--> One definition is not valid.
Axiom Schemas:
	--> All the axiom schemas are valid.
Rules of Transformation:
...
Theorem Schemas:
    T5: (p⊃(q⊃r))⊃((p⊃q)⊃(p⊃r))              importation
	   not valid: |p|=2, |q|=2, |r|=1, result=18
    T9: ((p⊃q)⊃p)⊃p                          Pierce
	   not valid: |p|=2, |q|=1, result=18
   T11: (p⊃(q⊃r))⊃((p∧q)⊃r)                  importation
	   not valid: |q|=2, |p|=2, |r|=1, result=18
   T13.1: p∨~p                                 tertium non datur
	   not valid: |p|=2, result=18
   T14: ~(p∧~p)                              contradiction
	   not valid: |p|=2, result=18
   T15.1: (p⊃~p)⊃~p                            reductio ad absurdum
	   not valid: |p|=2, result=18
...
Derived Rules:
   DR6: If ⊢(~p⊃p), then ⊢p.                 contradiction 1
	   Conclusion p = 6 when |p|=6 and premises hold.
   DR7: If ⊢(p⊃~p), then ⊢~p.                contradiction 2
	   Conclusion ~p = 18 when |p|=2 and premises hold.
	--> 2 transformations are not valid.
Total number of invalid items: 10
```
...
### Are ℒ(4) and Ł₃ isomorphic?

It's very easy to check that they are. After *run*ning module *divisor3.logic* in a Python *idle* session, we use the session to create the truth tables.  Since the remaining connectives can be defined in terms of negation and material implication, it suffices to show that we can simply relabel the values in the truth tables for ℒ(4) to obtain truth tables for Ł₃.

I am including the relevant Python code, so you can do this yourself.  (Remember that you need to run module *divisor3.logic* as a *main program* inside *idle* (or some other deveopment shell like *spyder*) and the Python code must follow in the same session!)

```
   >>> L4 = DivisorLogic(4)
       DivisorLogic::_set_lattice: L: top=4, bottom=1, |L|=3, t=2
   >>> lat = sorted(L4.lattice)
   >>> lat
       [1, 2, 4]
```
We will map 1 to F=0, 2 to I=1/2, and 4 to T=1.  We will call this map *f*. This map is a bijection, *i.e.* it is one-to-one and onto: (a) we can *f* to map values of expressions ℒ(4) to corresponding values of the same expressions in Ł₃; and (b) we can use its inverse to recover the values for an expression in ℒ(4) from the corresponding values in the same expression in Ł₃. For negation we have:
```
   >>> for p in lat:
   ...     print(p, L4.NOT(p))
   ...
       ...
       4 1
```
Let's draw the truth tables for negation:
```
        ℒ(4)           Ł₃  or   Ł₃
       p ~p  ---f---> p ~p    p    ~p
      ━━━━━━         ━━━━━━   ━━━━━━━━━━━
       ...
       4  1           T  F    1    0
             <--f⁻¹--
```
The tables are structurally the same.  The differ only in the labels for the components.

Now for material implication:
```
   >>> print_matrix("p⊃q", lat, L4.IMP)
       ...
       ━━━━━━━━━━━━━━━━━━━━━━━
              ...
              p= 4 ┃  1  2  4
```
Our map *f* is example of an isomorphism -- it tells us how to relabel everything.  Let's apply it to the truth table for material implication:
```
                  ℒ(4)                             Ł₃
        p⊃q ┃     q=  1  2  4  ---f--->  p⊃q ┃     q=  F  I  T
       ━━━━━━━━━━━━━━━━━━━━━━━          ━━━━━━━━━━━━━━━━━━━━━━━
              p= 1 ┃  4  4  4                  p= F ┃  T  T  T
              p= 2 ┃  2  4  4                  p= I ┃  I  T  T
              p= 4 ┃  1  2  4                  p= T ┃  F  I  T
...
Now let's check that this works for the definitions.  First the Python code and the responses:
```
   >>> print_matrix("p∨q", lat, L4.OR)
...
Now let's use the isomorphism to manually derive the tables in Ł₃:
```
                  ℒ(4)                             Ł₃
        p∨q ┃     q=  1  2  4  ---f--->  p∨q ┃     q=  F  I  T
       ━━━━━━━━━━━━━━━━━━━━━━━          ━━━━━━━━━━━━━━━━━━━━━━━
       ...
       ━━━━━━━━━━━━━━━━━━━━━━━          ━━━━━━━━━━━━━━━━━━━━━━━
              ...
              p= 4 ┃  1  2  4                  p= 4 ┃  F  I  T
```
The tables on the right are exactly what we wanted.

Easy peasy, right?  Well yes and no.  The problem of determining whether two sets of truth tables is NP-complete.  That means that it is easy to check (in polynomial time) whether a map is an isomorphism, but there is no known polynomial time algorithm for finding the isomorphism.  Moreover, if you can find such an algorithm, then you can use it to solve every NP problem in polynomial time.  (It is currently not known whether there is a polynomial algorithm which solves an NP-complete problem.)

### Are ℒ(8) and Ł₄ isomorphic?

Let's check.  (Figuring out the Python code that I used is left as an exercise.)

The map *f* that I will use takes 1 to F=0, 2 to I=1/3, 4 to J=2/3, and 8 to T=1.  I won't use the fractions -- the letters suffice.   The claim is that f is an isomorphism.  To check I simply need to reproduce the truth tables for Ł₄ using the truth tables for ℒ(8) and the conjectured isomorphism f.
...
```
     p⊃q ┃     q=  1  2  4  8      p⊃q ┃     q=  F  I  J  T
    ━━━━━━━━━━━━━━━━━━━━━━━━━━    ━━━━━━━━━━━━━━━━━━━━━━━━━━
           p= 1 ┃  8  8  8  8            p= F ┃  T  T  T  T
           p= 2 ┃  4  8  8  8            p= I ┃  J  T  T  T
           p= 4 ┃  2  4  8  8            p= J ┃  I  J  T  T
           p= 8 ┃  1  2  4  8            p= T ┃  F  I  J  T
```
The defined connectives:
```
     p∨q ┃     q=  1  2  4  8      p∨q ┃     q=  F  I  J  T
    ━━━━━━━━━━━━━━━━━━━━━━━━━━    ━━━━━━━━━━━━━━━━━━━━━━━━━━
           p= 1 ┃  1  2  4  8            p= F ┃  F  I  J  T
           p= 2 ┃  2  2  4  8            p= I ┃  I  I  J  T
           p= 4 ┃  4  4  4  8            p= J ┃  J  J  J  T
           p= 8 ┃  8  8  8  8            p= T ┃  T  T  T  T

     p∧q ┃     q=  1  2  4  8      p∧q ┃     q=  F  I  J  T 
    ━━━━━━━━━━━━━━━━━━━━━━━━━━    ━━━━━━━━━━━━━━━━━━━━━━━━━━
           p= 1 ┃  1  1  1  1            p= F ┃  F  F  F  F
           p= 2 ┃  1  2  2  2            p= I ┃  F  I  I  I
           p= 4 ┃  1  2  4  4            p= J ┃  F  I  J  J
           p= 8 ┃  1  2  4  8            p= T ┃  F  I  J  T

     p≡q ┃     q=  1  2  4  8      p≡q ┃     q=  F  I  J  T
    ━━━━━━━━━━━━━━━━━━━━━━━━━━    ━━━━━━━━━━━━━━━━━━━━━━━━━━
           p= 1 ┃  8  4  2  1            p= F ┃  T  J  I  F
           p= 2 ┃  4  8  4  2            p= I ┃  J  T  J  I
           p= 4 ┃  2  4  8  4            p= J ┃  I  J  T  J
           p= 8 ┃  1  2  4  8            p= T ┃  F  I  J  T
```
These are correct, so $f$ is an isomorphism.  ℒ(8) and Ł₄ are isomorphic.

### The general case
 
It should not be too difficult to prove that, with negation and implication defined as described above in section 7 (implication in bullet points (1), (2) and (3.3)), that ℒ(2ⁿ⁻¹) is isomorphic to Łₕ.